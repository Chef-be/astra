...
{	
	...
	$('#traderDescription').text(ShipDescriptions[shipID] || '');
	$('#availableCount').text(NumberGetHumanReadable(CostInfo[shipID][0]));
	Reset();
}
...
function Total()
{
	...
	var shipID 	= $('#shipID').val();
	if (Count > CostInfo[shipID][0]) {
		Count = CostInfo[shipID][0];
		$('#count').val(Count);
	}
