...
{
	protected $cronjobError = array();
	protected $disabledCronjobClasses = array('TeamSpeakCronjob');
	...
	function showCronjobDetail(){
		...
		foreach ($dir as $fileinfo) {
			if ($fileinfo->isFile() && $fileinfo->getBasename('.class.php') != $fileinfo->getFilename()) {
				$cronClass = $fileinfo->getBasename('.class.php');
				if (in_array($cronClass, $this->disabledCronjobClasses, true)) {
					continue;
				}
				$avalibleCrons[]	= $cronClass;
			}
		}
		...
		if (!$CronjobRow) {
			$this->printMessage('La tâche planifiée demandée est introuvable.');
		}
...
function delete(){
	...
	function edit(){
		...
		if (!empty($error)) {
			$this->cronjobError = $error;
			$this->showCronjobDetail();
			return;
		}
