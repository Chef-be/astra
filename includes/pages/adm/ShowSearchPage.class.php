		...
		if ($CountQuery['total'] > 0)
		{
			...
			while ($WhileResult	= $GLOBALS['DATABASE']->fetch_num($FinalQuery))
			{
				...
				if ($Table == "users")
				{
					...
					if ($USER['authlevel'] == AUTH_ADM)
					{
						$DELETEBUTTON = $WhileResult[0] != $USER['id'] || $WhileResult[0] != ROOT_USER ? '<a href="?page=search&mode=deleteUser&amp;user='.$WhileResult[0].'" border="0" onclick="return confirm(\''.$LNG['ul_sure_you_want_dlte'].' '.$WhileResult[1].'?\');"><img src="./styles/resource/images/alliance/CLOSE.png" width="16" height="16" title='.$WhileResult[1].'></a>' : '-';
				...
				if ($Table == "planets p"){
					...
					if ($USER['authlevel'] == AUTH_ADM)
						$Search['LIST']	.= '<td><a href="?page=search&mode=deletePlanet&amp;planet='.$WhileResult[0].'" border="0" onclick="return confirm(\''.$LNG['se_confirm_planet'].' '.$WhileResult[1].'\');"><img src="./styles/resource/images/alliance/CLOSE.png" width="16" height="16" title='.$LNG['button_delete'].'></a></td>';
				}
