...
class ShowAccountDataPage extends AbstractAdminPage
{
	...
	function show(){
		...
		$UserWhileLogin = Database::get()->select($sql,array(
...
function ShowAccountDataPage()
{
	...
	if (!empty($id_u))
	{
		...
		$sql = "SELECT `id`, `authlevel` FROM %%USERS%% WHERE `id` = :userId AND `universe` = :universe;";

		$OnlyQueryLogin = Database::get()->selectSingle($sql,array(
			':userId' => $id_u,
			':universe' => Universe::getEmulated()
		));
		...
		else
		{
			...
			$sql = "SELECT " . $SpecifyItemsU . " FROM %%USERS%% as u LEFT JOIN
			%%ALLIANCE%% as a ON a.id = u.ally_id WHERE
			u.id = :userId;";

			$UserQuery = Database::get()->selectSingle($sql,array(
				':userId' => $id_u
			));
			...
			if ($UserQuery['bana'] != 0)
			{
				...
				$BannedQuery = Database::get()->selectSingle($sql,array(
			...
			$sql = "SELECT " . $SpecifyItemsS . " FROM %%STATPOINTS%% WHERE `id_owner` = :userId AND `stat_type` = '1';";

			$StatQuery = Database::get()->selectSingle($sql,array(
				':userId' => $id_u
			));
			...
			elseif ($alianza != NULL && $AliID != 0)
			{
				...
				$AllianceQuery = Database::get()->selectSingle($sql,array(
					':ally_name' => $alianza
				));
				...
				{
					$ali_ext2 = RichTextService::render($AllianceQuery['ally_description']);
				...
				{
					$ali_int2 = RichTextService::render($AllianceQuery['ally_text']);
				...
				{
					$ali_sol2 = RichTextService::render($AllianceQuery['ally_request']);
				...
				$SearchLeader = Database::get()->selectSingle($sql,array(
				...
				$StatQueryAlly = Database::get()->selectSingle($sql,array(
				 ':ally_leader' => $ali_lider
				));
			...
			$PlanetsQuery = Database::get()->select($sql,array(
				':userId' => $id_u
			));
