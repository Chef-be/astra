...
class ShowMessagesPage extends AbstractGamePage
{
    ...
    function show()
    {
        ...
        $allowedIncomingCategories = array(1, 2);
        ...
        $messageCategory = HTTP::_GP('category', 1);
        $messagePage = HTTP::_GP('side', 1);
        ...
        if (!isset($availableCategories[$messageCategory])) {
            $messageCategory = 1;
        }

        $sql = "SELECT COUNT(*) as state FROM %%MESSAGES%%
        WHERE message_sender = :userID AND message_type IN (1, 2) AND message_deleted IS NULL;";
        ...
        WHERE message_owner = :userID AND message_deleted IS NULL AND message_type IN (1, 2) GROUP BY message_type;";
        ...
        foreach($availableCategories as $CategoryID => $CategoryColor) {
      ...
      else
      {
        ...
        foreach ($MessageResult as $MessageRow)
        {
            ...
            $MessageList[]	= array(
                ...
                'text'		=> RichTextService::render($MessageRow['message_text']),
            );
        }
