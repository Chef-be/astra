...
class ShowOfficierPage extends AbstractGamePage
{
	...
	private function formatElementDescription($elementId, array $elementBonus, $lng)
	{
		$description = isset($lng['shortDescription'][$elementId]) ? $lng['shortDescription'][$elementId] : '';

		if ($description === '' || strpos($description, '%s') === false) {
			return $description;
		}

		$firstBonus = reset($elementBonus);
		$bonusValue = 0;

		if (is_array($firstBonus) && isset($firstBonus[0], $firstBonus[1])) {
			$bonusValue = $firstBonus[1] == 0 ? abs($firstBonus[0] * 100) : abs($firstBonus[0]);
		}

		return sprintf($description, $bonusValue);
	}
	...
	public function show()
	{
		...
		if(isModuleAvailable(MODULE_DMEXTRAS))
		{
			foreach($reslist['dmfunc'] as $Element)
			{
				...
				$darkmatterList[$Element]	= array(
					...
					'description'		=> $this->formatElementDescription($Element, $elementBonus, $LNG),
				);

				if($darkmatterList[$Element]['timeLeft'] > 0) {
					$activeDmCount++;
				}
			}
		}
		...
		if(isModuleAvailable(MODULE_OFFICIER))
		{
			foreach($reslist['officier'] as $Element)
			{
				...
				if($officierList[$Element]['level'] >= $officierList[$Element]['maxLevel']) {
					$maxedOfficerCount++;
				}
			}
		}
		...
		$this->assign(array(
			...
			'dmCount'			=> count($darkmatterList),
			'officerCount'		=> count($officierList),
			'activeDmCount'		=> $activeDmCount,
			'maxedOfficerCount'	=> $maxedOfficerCount,
