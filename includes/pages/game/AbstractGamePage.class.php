...
abstract class AbstractGamePage
{
	...
	protected function getNavigationData()
	{
		...
		$playerUnreadMessages = (int) $db->selectSingle("SELECT COUNT(*) AS count
			FROM %%MESSAGES%%
			WHERE message_owner = :userId
				AND message_deleted IS NULL
				AND message_unread = 1
		...
		$this->assign(array(
			...
			'messages'					=> ($playerUnreadMessages > 0) ? (($playerUnreadMessages == 1) ? $LNG['ov_have_new_message'] : "(" . $playerUnreadMessages . ")"): false,
		));
	}
