...
class ShowTicketPage extends AbstractGamePage
{
	...
	function send()
	{
		...
		$db = Database::get();
		$adminUsers = $db->select("SELECT id FROM %%USERS%% WHERE authlevel > :authLevel AND universe = :universe;", array(
			':authLevel' => AUTH_USR,
		...
		foreach ($adminUsers as $adminUser) {
			NotificationService::createSupportTicketNotification(
				...
				$USER['id'],
				$ticketID,
				$isNewTicket ? 'Nouveau ticket de support' : 'Ticket de support mis à jour',
				$isNewTicket
					? $USER['username'].' a créé le ticket #'.$ticketID.'.'
					: $USER['username'].' a ajouté un message au ticket #'.$ticketID.'.',
				'support_ticket',
				Universe::current()
			);
		}
