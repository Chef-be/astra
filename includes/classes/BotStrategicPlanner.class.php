...
class BotStrategicPlanner
{
	...
	protected function computeNeeds(array $snapshot, array $config, array $hierarchy)
	{
		...
		$scores['besoin_defensif'] = min(100, ($incomingThreat * 0.40) + (($zoneDissuasion + $globalDefensiveBias) * 0.25) + ((isset($traits['defense']) ? $traits['defense'] : 50) * 0.15) + ((isset($dynamic['vigilance']) ? $dynamic['vigilance'] : 50) * 0.10) + (max(0, 100 - $commanderCohesion) * 0.10));
		$scores['besoin_expansion'] = min(100, ($freeFields < 25 ? 45 : 15) + ((isset($traits['expansion']) ? $traits['expansion'] : 50) * 0.25) + (max(0, 4 - $planetCount) * 15) + ($campaignScore * 0.10) + ($zoneProfitability * 0.20) + (max(0, 60 - $territorialImportance) * 0.15));
		...
		$scores['besoin_militaire'] = min(100, (max($opportunity, $territorialControl) * 0.24) + ((isset($traits['intensite_offensive']) ? $traits['intensite_offensive'] : 50) * 0.18) + ($campaignScore * 0.14) + ((isset($dynamic['superiorite_locale']) ? $dynamic['superiorite_locale'] : 50) * 0.12) + (($globalOffensiveBias + $territorialImportance + $zonePressureMemory) * 0.32));
		$scores['besoin_surveillance_active'] = min(100, (($zoneCoverage + $globalCoverageBias) * 0.28) + ($zonePressure * 0.14) + ($psychologicalPressure * 0.10) + ((isset($traits['espionnage']) ? $traits['espionnage'] : 50) * 0.16) + ((isset($dynamic['vigilance']) ? $dynamic['vigilance'] : 50) * 0.08) + ((isset($traits['sens_opportunite']) ? $traits['sens_opportunite'] : 55) * 0.12) + ($profileEfficiency * 0.12));
		...
		$scores['besoin_pression_psychologique'] = min(100, ($psychologicalPressure * 0.24) + ($campaignScore * 0.14) + ($zonePressure * 0.10) + ($communicationReadiness * 0.08) + ((isset($traits['aptitude_communication']) ? $traits['aptitude_communication'] : 50) * 0.10) + ((isset($traits['aptitude_intimidation']) ? $traits['aptitude_intimidation'] : 40) * 0.18) + ((isset($traits['agressivite_verbale']) ? $traits['agressivite_verbale'] : 35) * 0.08) + ($targetPressureMemory * 0.08));
	...
	protected function chooseIntention($needKey, $goal, array $snapshot, array $hierarchy)
	{
		...
		if (!empty($hierarchy['state']['current_target_json']) && in_array($goal, array('pression_locale', 'surveillance_active'), true)) {
			return 'tester_cible';
		}
