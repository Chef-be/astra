...
class PlayerUtil
{
	...
	static public function sendMessage($userId, $senderId, $senderName, $messageType, $subject, $text, $time, $parentID = NULL, $unread = 1, $universe = NULL)
	{
		...
		if ((int) $unread === 1 && in_array((int) $messageType, array(0, 3, 4, 5, 15, 50), true)) {
			NotificationService::createFromMessage($userId, $messageType, $subject, $text, 'game.php?page=messages&category='.$messageType, $universe);
		}
	}
}
