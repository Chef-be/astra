...
<div class="container-fluid py-3 text-white">
	<div class="admin-mini-hero mb-4">
		<h2 class="h3">Droits d’administration</h2>
		<p>Affectez rapidement un niveau d’accès ou filtrez les comptes déjà classés par rôle.</p>
	</div>

	<div class="admin-card">
		<div class="admin-card__body">
			<form action="?page=rights&mode=usersSend" method="post" name="users">
				<div class="admin-actions mb-3">
					<a class="btn btn-outline-light btn-sm" href="?page=rights&sid={$sid}&type=adm">{$LNG.ad_authlevel_aa}</a>
					<a class="btn btn-outline-light btn-sm" href="?page=rights&sid={$sid}&type=ope">{$LNG.ad_authlevel_oo}</a>
					<a class="btn btn-outline-light btn-sm" href="?page=rights&sid={$sid}&type=mod">{$LNG.ad_authlevel_mm}</a>
					<a class="btn btn-outline-light btn-sm" href="?page=rights&sid={$sid}&type=pla">{$LNG.ad_authlevel_jj}</a>
					<a class="btn btn-outline-info btn-sm" href="?page=rights&sid={$sid}">{$LNG.ad_authlevel_tt}</a>
				</div>
