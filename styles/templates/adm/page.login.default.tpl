...
function loginSubmit(activeRecaptcha,use_recaptcha_on_admin_login){
	...
	<form style="max-width:400px;margin-top:120px;" action="?page=login&mode=validate" method="post" class="mx-auto p-4 bg-black rounded-4 border border-secondary shadow-lg">
	<div class="text-center text-white fs-3 fw-bold mb-3">
		{if $brandLogoUrl}
			<img src="{$brandLogoUrl}" alt="{$gameName|default:'Astra Dominion'}" style="max-width:220px;max-height:72px;width:auto;height:auto;">
		{else}
			{$gameName|default:'Astra Dominion'}
		{/if}
	</div>
