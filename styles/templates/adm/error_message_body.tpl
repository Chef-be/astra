...
{assign var="adminTemplateRedirectTarget" value=""}
{if !empty($goto)}
	{assign var="adminTemplateRedirectTarget" value=$goto}
{/if}
<section class="admin-message-shell">
	<article class="admin-message-card">
		<div class="admin-message-card__header">
			<span class="admin-pill">{$fcm_info}</span>
			<h2 class="admin-message-card__title">{$fcm_info}</h2>
		</div>
		<div class="admin-message-card__body">{$mes}</div>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	var redirectTarget = '{$adminTemplateRedirectTarget|escape:"javascript"}';
